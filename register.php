...
if($_POST)
{
  ...
  if(!sessionCheck('level','dean'))
  {
    ...
    if($query->fetch()[0])
      postResponse("error", "You are not authorized to register accounts. Contact the Dean of your faculty");
    $newAdmin = 1;
  }
  ...
  if(!empty($_POST['dept'])) //Register new faculty (and new department if required)
  {
    ...
    $query = $db->prepare('INSERT INTO faculty(uName,fac_name,pswd,email,dept_code,dateRegd) VALUES (?,?,?,?,?,?)');
